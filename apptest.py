...
from exts import create_db,create_app
from models import Web_listen
async_mode = None
...
def background_thread():
    ...
    while True:
        ...
        web = Web_listen(date=str([t, cpus, process, net_in/5, net_out/5, net_all/5]))
        print(cpus, process)
        db.session.add(web)
...
# @app.route('/')
# def index():
#     # web = Web_listen(time='11', cpu=1, memery=1, web_in=1, web_out=5, web_all=5)
#     # print(web)
#     # db.session.add(web)
#     # db.session.commit()
#     return 'asd'
